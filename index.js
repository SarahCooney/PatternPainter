...
function setupWorld() {
  new THREE.TextureLoader().load('./images/sky.jpg', function(t1) {
  ...
  water = new THREE.Mesh(
	...
	//new THREE.PlaneBufferGeometry(2048+1024, 2048+1024, 16, 16),
    new THREE.MeshLambertMaterial({color: 0x292b31, transparent: true, opacity: 0.8})
  );
  water.position.y = -99;
...
//TERRAIN SETTINGS 
  function Settings() {
   ...
   var elevationGraph = document.getElementById('elevation-graph'),  //Analytics Variables
        slopeGraph = document.getElementById('slope-graph'),
        analyticsValues = document.getElementsByClassName('value');
		
    var loader = new THREE.TextureLoader(); //Creates base-floor 
    loader.load('./images/sidewalk_1.jpg', function(t1) {
     ...
     loader.load('./images/sidewalk_1.jpg', function(t2) {
    ...
    this.spread = 60;
    this.scattering = 'PerlinAltitude';  //Scattering of trees 
	...
	//Regeneration of the screen / settings 
    window.rebuild = this.Regenerate = function() {
      ...
      skyDome.visible = road.visible = water.visible = that.texture != 'Wireframe';  
    ...
    this.altitudeSpread = function(v, k) {  //How to create the spread function
      return k % 4 === 0 && Math.random() < altitudeProbability(v.z);
    };
	
    var mesh = buildTree();  //The object to scatter 
	
    var decoMat = mesh.material.map(  //Setup the material of the mesh 
      function(mat) {
        return mat.clone();
    ...
    this['Scatter meshes'] = function() { //Function to scatter the trees 
      ...
      var geo = terrainScene.children[0].geometry; //The geometry of the terrain - needed for scatter meshes. 
      terrainScene.remove(decoScene);
      decoScene = THREE.Terrain.ScatterMeshes(geo, { //Options
        mesh: mesh, //The object 
        w: s, //width = # segments
        h: Math.round(s * that['width:length ratio']), //height 
        spread: spread, //Spread, defined by the function or not
        smoothSpread: that.scattering === 'Linear' ? 0 : 0.2, //Goes with a numeric spread 
  ...
  //Recreate this for each pattern. 
  ...
  patternFolder.add(settings, 'Add Tree').onChange(function(val) { 
	trees = val;
	if(trees) {
		mesh = buildTree();  //The object to scatter 
	
		decoMat = mesh.material.map(  //Setup the material of the mesh 
			function(mat) {
			return mat.clone();
		}); // new THREE.MeshBasicMaterial({color: 0x229966, wireframe: true});	  
		decoMat[0].wireframe = true;
		...
		terrainScene.remove(testScene);
		testScene = THREE.Terrain.PlaceMeshes(geo, { //Options
        mesh: mesh, //The object 
        w: s, //width = # segments
        h: Math.round(s), // * that['width:length ratio']), //height 
        i: 7,
      ...
      if (testScene) {
        /*if (that.texture == 'Wireframe') {
          testScene.children[0].material = decoMat;
        }
        else if (that.texture == 'Grayscale') {
          testScene.children[0].material = gray;
        } */
        terrainScene.add(testScene);
      }
...
function placeTree(v, k) { return v.x > 0 && !(k % 4); }
